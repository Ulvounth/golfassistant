...
/**
 * Modal for å legge til ny golfbane
 * Lar bruker fylle inn info fra scorecard
 * Lagrer draft i localStorage for å unngå datatap
 */
export function AddCourseModal({ isOpen, onClose, onCourseAdded }: AddCourseModalProps) {
  ...
  // Lagre draft automatisk når data endres
  useEffect(() => {
  ...
  const handleBasicInfoSubmit = (e: FormEvent) => {
    ...
    if (!formData.name || !formData.location) {
      setError('Navn og lokasjon er påkrevd');
      return;
    }
    setError('');
    setStep(2);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      // Slett draft når lagring var vellykket
      clearDraft();
      ...
      // Hvis 401 (token utløpt), ikke vis feilmelding
      // Axios interceptor håndterer redirect og draft er allerede lagret
      if (error.response?.status === 401) {
        return;
      }

      setError(error.response?.data?.message || 'Kunne ikke opprette bane');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white border-b px-6 py-4 flex justify-between items-center">
          <h2 className="text-2xl font-bold">Legg til ny golfbane</h2>
          ...
          {hasDraft && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="font-semibold text-blue-900 mb-1">💾 Ulagret utkast funnet</h3>
                  <p className="text-sm text-blue-700 mb-3">
                    Du har et ulagret utkast fra tidligere. Vil du fortsette der du slapp?
                  </p>
                  <div className="flex gap-3">
                    <button onClick={loadDraft} className="btn-primary text-sm">
                      Ja, fortsett med utkast
                    </button>
          ...
          {isExpiringSoon && minutesUntilExpiry !== null && (
            <div className="bg-orange-50 border border-orange-200 rounded-lg p-4 mb-4 flex items-start gap-3">
              ...
              <div className="text-sm">
                <p className="font-semibold text-orange-900 mb-1">⏰ Økten din utløper snart</p>
                <p className="text-orange-700">
                  Du har {minutesUntilExpiry} minutter igjen. Dataene lagres automatisk, så du kan
                  fortsette etter innlogging.
                </p>
              </div>
            </div>
          )}
            ...
            <form onSubmit={handleBasicInfoSubmit} className="space-y-6">
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <p className="text-sm text-blue-800">
                  ℹ️ <strong>Tips:</strong> Registrer alle 18 hullene på banen. Når du skal spille
                  en runde kan du velge om du vil spille front 9, back 9, eller alle 18 hull.
                </p>
              </div>
            ...
            <form onSubmit={handleSubmit}>
              <div className="mb-6">
                <p className="text-gray-600 mb-4">
                  Fyll inn par og lengde for hvert hull fra scorecardet. Du kan også bare bruke
                  standard-verdiene.
                </p>
              </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b">
                      <th className="text-left py-2">Hull</th>
